...
export default function Reports() {
  // ============ STATE MANAGEMENT ============
  // 🔥 FIXED: Use fetchSubmissions instead of fetchAllSubmissions
  const { organizations, getAllSubmissions, fetchOrganizations, fetchSubmissions } = useAppStore();
  ...
  const loadData = async () => {
    ...
    try {
      await Promise.all([
        fetchOrganizations(),
        fetchSubmissions() // 🔥 FIXED: Use fetchSubmissions
      ]);
    } catch (error) {
  ...
  // 🔥 HELPER: GET INDICATOR VALUE (HANDLE BOTH FORMATS)
  const getIndicatorValue = (submission: any, indicatorNumber: string): number => {
    // Handle array format
    if (Array.isArray(submission.indicators)) {
      const indicator = submission.indicators.find((ind: any) => ind.number === indicatorNumber);
      return indicator ? (parseFloat(String(indicator.value)) || 0) : 0;
    }
    
    // Handle object format
    if (submission.indicators && typeof submission.indicators === 'object') {
      const value = submission.indicators[indicatorNumber];
      return value ? (parseFloat(String(value)) || 0) : 0;
    }
  ...
  const calculateSystemMetrics = () => {
    ...
    allSubmissions.forEach(submission => {
      totalSTRs += getIndicatorValue(submission, '1.1');
      totalCases += getIndicatorValue(submission, '6.1');
      totalAssetsFrozen += getIndicatorValue(submission, '6.4');
      totalAssetsSeized += getIndicatorValue(submission, '6.5');
      totalConvictions += getIndicatorValue(submission, '6.3');
      totalEnforcementActions += getIndicatorValue(submission, '3.3');
    });
  ...
  const monthlyTrend = Array.from({ length: 12 }, (_, i) => {
    ...
    monthSubmissions.forEach(sub => {
      strs += getIndicatorValue(sub, '1.1');
      cases += getIndicatorValue(sub, '6.1');
    });
  ...
  const typeComplianceData = Object.entries(complianceByType).map(([type, data]) => ({
    ...
    compliance: data.count > 0 ? Math.round(data.compliance / data.count) : 0,
    submissions: data.total
  }));

  // 🔥 UPPERCASE STATUS - Status distribution
  const statusData = [
    { name: 'Approved', value: allSubmissions.filter(s => s.status === 'APPROVED').length, color: '#10b981' },
    { name: 'Submitted', value: allSubmissions.filter(s => s.status === 'SUBMITTED' || s.status === 'UNDER_REVIEW').length, color: '#3b82f6' },
  ...
  // 🔥 FIXED EXPORT HANDLER WITH PROPER PARAMS
  const handleExport = async (format: 'csv' | 'pdf' | 'excel') => {
    setIsExporting(true);
    toast.loading(`Exporting ${activeReport} report as ${format.toUpperCase()}...`);
    
    try {
      // 🔥 FIXED: Properly structure the export params
      ...
      if (response.data.success) {
        ...
        if (format === 'csv') {
          ...
          switch (activeReport) {
            case 'compliance':
              csvData = organizations.map(org => {
                const orgSubs = allSubmissions.filter(s => s.organizationId === org.id);
                const approved = orgSubs.filter(s => s.status === 'APPROVED').length;
                return {
                  'Code': org.code,
                  'Name': org.name,
                  'Total': orgSubs.length,
                  'Approved': approved,
                  'Pending': orgSubs.filter(s => s.status === 'SUBMITTED' || s.status === 'UNDER_REVIEW').length,
            ...
            case 'financial':
              csvData = [{
                'Total STRs': systemMetrics.totalSTRs,
                'Active Cases': systemMetrics.totalCases,
                'Assets Frozen': formatCurrency(systemMetrics.totalAssetsFrozen),
                'Assets Seized': formatCurrency(systemMetrics.totalAssetsSeized),
                'Convictions': systemMetrics.totalConvictions,
                'Enforcement Actions': systemMetrics.totalEnforcementActions
              }];
              break;
          ...
          filename = `FIA_${activeReport}_report_${new Date().toISOString().split('T')[0]}.csv`;
        } else {
          // For PDF/Excel, use backend response
          blob = new Blob([JSON.stringify(response.data.data, null, 2)], { 
            type: format === 'pdf' ? 'application/pdf' : 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' 
          });
          filename = `FIA_${activeReport}_report_${new Date().toISOString().split('T')[0]}.${format}`;
        }
  ...
  // 🔥 FIXED SCHEDULE REPORT WITH API
  const handleScheduleReport = async (schedule: ReportSchedule) => {
    try {
      ...
      if (response.data.success) {
        toast.success(`Report scheduled successfully! You'll receive ${activeReport} reports ${schedule.frequency}.`);
      ...
      <div className="space-y-6">
        ...
        {isLoading && (
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">Loading report data...</h3>
                    ...
                    <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                      ...
                      <p className="text-3xl font-bold text-green-700">
                        {organizations.filter(org => {
                          ...
                          return subs.length > 0 && subs.filter(s => s.status === 'APPROVED').length === subs.length;
                        }).length}
                      </p>
                    </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {organizations.map(org => {
                          const orgSubs = allSubmissions.filter(s => s.organizationId === org.id);
                          const approved = orgSubs.filter(s => s.status === 'APPROVED').length;
                          const pending = orgSubs.filter(s => s.status === 'SUBMITTED' || s.status === 'UNDER_REVIEW').length;
                        ...
                        [
                          { metric: 'Compliance', ...selectedOrgs.reduce((acc, orgId) => {
                            ...
                            if (org) {
                              ...
                              acc[org.code] = subs.length > 0 ? Math.round((subs.filter(s => s.status === 'APPROVED').length / subs.length) * 100) : 0;
                            }
                            return acc;
                          }, {} as any) },
                          ...
                          { metric: 'Completeness', ...selectedOrgs.reduce((acc, orgId) => {
                            ...
                            if (org) {
                              const subs = allSubmissions.filter(s => s.organizationId === orgId);
                              acc[org.code] = subs.length > 0 ? Math.round(subs.reduce((sum, s) => sum + (s.completionRate || 0), 0) / subs.length) : 0;
                            }
                            return acc;
                          }, {} as any) },
